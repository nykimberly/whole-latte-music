(function () {
    ...
    // Write functions
    setSong = function (number) {                                                   // Assigns two variables with song number.
        if (currentSoundFile) {
            currentSoundFile.stop();                                                // Pause any song currently playing.
        }
    ...
    seek = function (time) {                                                        // Function to change position in song to specific time.
        if (currentSoundFile) {
            currentSoundFile.setTime(time);
        }
    };

    setVolume = function (volume) {                                                 // Function to set volume
        if (currentSoundFile) {
    ...
    createSongRow = function (songNumber, songName, songLength) {                   // Create a function that generates the the content for each row
        template =
    ...
    setCurrentAlbum = function (album) {
        ...
        $albumSongList.empty();
        for (i = 0; i < album.songs.length; i += 1) {                               // Assign album.song[index] to its corresponding song#, name, and length
    ...
    updateSeekPercentage = function ($seekBar, seekBarFillRatio) {                  // Write a generic function to update any seek bar with seekBarFillRatio
